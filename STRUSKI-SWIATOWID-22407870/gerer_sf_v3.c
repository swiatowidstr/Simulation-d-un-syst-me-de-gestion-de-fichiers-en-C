...
static void TestEcrireLireDonneesInode() {
    ...
    printf("\n");
		...
		//tests du décalage de l'écriture d'inode
		ecrits = EcrireDonneesInode(inode, (unsigned char*)test, strlen(test), 0);
   	ecrits = EcrireDonneesInode(inode, (unsigned char*)test, strlen(test), 10);
   	ecrits = EcrireDonneesInode(inode, (unsigned char*)test, strlen(test), 100);
   	ecrits = EcrireDonneesInode(inode, (unsigned char*)test, strlen(test), 600);
   	AfficherInode(inode);		

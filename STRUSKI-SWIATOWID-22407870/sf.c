...
static tSuperBloc CreerSuperBloc(char nomDisque[]) {
	...
	if (superBloc == NULL) {
		perror("CreerSuperBloc: création du super bloc a échoué");
		return NULL;
	}
...
tSF CreerSF (char nomDisque[]){
  ...
  if (sf == NULL) {
  	perror("CreerSF: allocation du sf a échoué");
  	return NULL;
  }
  ...
  if (sf->superBloc == NULL) {
  	free(sf);
  	perror("CreerSF: création du superBloc a échoué");
  	return NULL;
  }
...
long Ecrire1BlocFichierSF(tSF sf, char nomFichier[], natureFichier type) {
  if (sf == NULL || nomFichier == NULL) {
  	perror("Ecrire1BlocFichierSF: sf ou nomFichier inexistant");
  	return -1;
  }
  ...
  if (f == NULL) {
  	perror("Ecrire1BlocFIhierSF: ouverture du fichier à échoué");
  	return -1;
  }
...
long EcrireFichierSF(tSF sf, char nomFichier[], natureFichier type) {
	if (sf == NULL || nomFichier == NULL) {
		perror("EcrireFichierSF: sf ou nomFIchier inexistant");
		return -1;
	}
	...
	if (nouvelElement == NULL) {
		perror("EcrireFichierSF: création d'un element de la liste d'inodes a échoué");
		return -1;
	}
	...
	FILE *f = fopen(nomFichier, "rb");
	if (f == NULL) {
		perror("EcrireFichierSF: ouverture fichier a échoué");
		return -1;
	}
...
int SauvegarderSF(tSF sf, char nomFichier[]) {
	if (sf == NULL || nomFichier == NULL) {
		perror("SauvegarderSF: sf ou nomFichier inexistant");
		return -1;
	}
	
  FILE *f = fopen(nomFichier, "wb");
  if (f == NULL) {
  	perror("SauvegarderSF: ouverture fichier à échoué");
  	return -1;
  }
...
int ChargerSF(tSF *pSF, char nomFichier[]) {
  if (nomFichier == NULL || pSF == NULL) {
  	perror("ChargerSF: nomFichier ou pSF inexistant");
  	return -1;
  }
  
  FILE *f = fopen(nomFichier, "rb");
  if (f == NULL) {
  	perror("ChargerSF: ouverture fichier à échoué");
  	return -1;
  }
  ...
  tSF sf = malloc(sizeof(struct sSF));
  if (sf == NULL) {
  	perror("ChargerSF: allocation sf a échoué");
  	fclose(f);
  	return -1;
  }
  ...
  if (sf->superBloc == NULL) {
  	perror("ChargerSF: allocation superBloc a échoué");
  	free(sf);
  	fclose(f);
  	return -1;
	}
  ...
  for (int i = 0; i < nbInodes; i++) {
  	...
  	if (element == NULL) {
  		perror("ChargerSF: création nouvel élément dans la liste chainée a échoué");
  		DetruireSF(&sf);
  		fclose(f);
  		return -1;
  	}

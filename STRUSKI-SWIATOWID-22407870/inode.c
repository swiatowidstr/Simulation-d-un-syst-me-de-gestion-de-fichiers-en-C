...
long EcrireDonneesInode(tInode inode, unsigned char *contenu, long taille, long decalage) {
  if (contenu == NULL || inode == NULL || taille <= 0) {
  	perror("EcrireDonneesInode: contenu ou inode non existants");
  	return -1;
  }
  
  if (taille <= 0) {
  	perror("EcrireDonneesInode: taille <= 0");
  	return -1;
  }
  ...
  if (decalage >= tailleMax) {
  	perror("EcrireDonneesInode: decalage >= a la taille max d'un fichier");
  	return 0;
  }
  ...
  for (int i = premierBloc; i <= dernierBloc; i++) {
  	if (inode->blocDonnees[i] == NULL) {
  		...
  		if (inode->blocDonnees[i] == NULL) {
  			perror("EcrireDonneesInode: problème création bloc");
  			return -1;
  		}
  	}
  }
...
int SauvegarderInode(tInode inode, FILE *fichier) {
  if (fichier == NULL || inode == NULL) {
  	perror("SauvegarderInode: fichier ou inode inexistant");
  	return -1;
  }
...
int ChargerInode(tInode *pInode, FILE *fichier) {
  if (fichier == NULL || pInode == NULL) {
  	perror("ChargerInode: fichier ou pIonde Inexistant");
  	return -1;
  }
